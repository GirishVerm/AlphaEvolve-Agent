#!/usr/bin/env python3
"""
Improved Dual-Loop Evolution System - Addresses all the identified issues.
"""
import asyncio
import logging
import time
...
class EvolutionConfig:
    """Configuration for improved dual-loop evolution."""
    artifact_population_size: int = 5  # μ (parent population)
    artifact_children_per_parent: int = 3  # λ (children per parent)
    scaffold_population_size: int = 3
    generations: int = 10
    max_cost: float = 50.0
    checkpoint_interval: int = 5

class ImprovedDualLoopEvolutionSystem:
    """Improved dual-loop evolution with μ→λ selection and better hygiene."""
    
    def __init__(self, task: TaskDefinition, config: EvolutionConfig):
        """Initialize the improved dual-loop evolution system."""
        self.task = task
        self.config = config
        self.generation = 0
    ...
    async def generate_hard_eval_suite(self) -> HardEvalSuite:
        """Generate comprehensive evaluation suite with better JSON parsing."""
        prompt = f"""
        Create a comprehensive evaluation suite for: {self.task.outcome}
        
        Task stubs: {self.task.stubs}
        Performance target: {self.task.performance_target}ms
        
        Return ONLY a valid JSON object (no extra text, no explanations):
        ```json
        {{
            "unit_tests": [
                ...
                {{"input": "1. item", "expected": "<ol><li>item</li></ol>", "name": "numbered_list"}},
                {{"input": "[link](url)", "expected": "<a href=\"url\">link</a>", "name": "basic_link"}}
            ],
            "performance_benchmarks": [
                {{"name": "small_text", "input": "# Test", "max_time": {self.task.performance_target}}},
                {{"name": "medium_text", "input": "# " + "x" * 100, "max_time": {self.task.performance_target * 5}}},
                {{"name": "large_text", "input": "# " + "x" * 1000, "max_time": {self.task.performance_target * 10}}}
            ],
            "robustness_tests": [
                {{"name": "empty_input", "input": "", "should_handle": true}},
                {{"name": "null_input", "input": null, "should_handle": true}},
                {{"name": "unicode_input", "input": "مرحبا بالعالم", "should_handle": true}},
                {{"name": "html_injection", "input": "<script>alert('xss')</script>", "should_handle": true}}
            ],
            "edge_cases": [
                ...
                "Empty lists: - ",
                "Malformed links: [broken](url",
                "Special characters: !@#$%^&*()",
                "Very long lines: " + "x" * 1000
            ]
        }}
        ```
        """
    ...
    def _create_basic_eval_suite(self) -> HardEvalSuite:
        ...
        return HardEvalSuite(
            ...
            performance_benchmarks=[
                {"name": "small_text", "input": "# Test", "max_time": self.task.performance_target},
                {"name": "large_text", "input": "# " + "x" * 100, "max_time": self.task.performance_target * 10}
            ],
            robustness_tests=[
                {"name": "empty_input", "input": "", "should_handle": True},
    ...
    async def initialize_populations(self):
        ...
        print("🔧 INITIALIZING IMPROVED DUAL-LOOP POPULATIONS")
        ...
        self.scaffold_population = {
            'prompts': [
                ...
                "Handle nested lists and footnotes."
            ],
            'tools': [
...
'''
    ...
    async def run_improved_dual_loop_evolution(self):
        """Run the improved dual-loop evolution."""
        print("🚀 STARTING IMPROVED DUAL-LOOP EVOLUTION")
        print("="*60)
        print("μ→λ Evolution + Meta-Evolution with Better Hygiene")
        print("="*60)
        ...
        for gen in range(self.config.generations):
            print(f"\n{'='*60}")
            print(f"🔄 IMPROVED DUAL-LOOP GENERATION {gen + 1}/{self.config.generations}")
            print(f"{'='*60}")
        ...
        # Show final results
        await self.show_improved_final_results()
    ...
    async def _checkpoint(self):
        ...
        print(f"\n💾 CHECKPOINTING GENERATION {self.generation}")
        # In a real system, this would save to disk/database
        pass
    
    async def show_improved_final_results(self):
        """Show comprehensive final results with proper code display."""
        print(f"\n{'='*60}")
        print("🎯 IMPROVED DUAL-LOOP EVOLUTION COMPLETE - FINAL RESULTS")
        print(f"{'='*60}")
...
async def main():
    """Run the improved dual-loop evolution demo."""
    print("🎭 IMPROVED DUAL-LOOP EVOLUTION DEMO")
    print("="*60)
    print("μ→λ Evolution + Meta-Evolution with Better Hygiene")
    print("="*60)
    ...
    # Initialize system
    config = EvolutionConfig(generations=3)  # Reduced for demo
    system = ImprovedDualLoopEvolutionSystem(task, config)
    
    # Run improved dual-loop evolution
    await system.run_improved_dual_loop_evolution()
