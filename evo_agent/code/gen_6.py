    ...
    async def watch(self):
        while not self._stopping:
            try:
                ...
                # Only sleep when no new data was read to minimize latency
                if not read_any:
...
class AnomalyDetector:
    ...
    """
    def __init__(self):
        # Maintain two time windows for O(1) counts per pattern
        # pattern_id -> deque of timestamps (5m window)
        self.timestamps_5m = defaultdict(lambda: deque())
        # pattern_id -> deque of timestamps (1m window)
        self.timestamps_1m = defaultdict(lambda: deque())

    def record(self, pat_id, ts=None):
        now = ts or time.time()
        dq5 = self.timestamps_5m[pat_id]
        dq1 = self.timestamps_1m[pat_id]
        dq5.append(now)
        dq1.append(now)
        cutoff5 = now - 300
        cutoff1 = now - 60
        while dq5 and dq5[0] < cutoff5:
            dq5.popleft()
        while dq1 and dq1[0] < cutoff1:
            dq1.popleft()

    def z_score(self, pat_id):
        # Approximate z-score using Poisson assumption: var ≈ mean
        dq5 = self.timestamps_5m[pat_id]
        dq1 = self.timestamps_1m[pat_id]
        cnt_5m = len(dq5)
        cnt_1m = len(dq1)
        mean5 = cnt_5m / 5.0  # per-minute baseline
        if mean5 <= 0:
            return 0.0
        std5 = (mean5) ** 0.5
...
class LogAnalyzer:
    ...
    async def start(self):
        ...
        # start worker (more workers to reduce queue latency)
        worker_count = max(8, len(self.config.log_files) * 2)
...
def main():
    ...
    # Create and set a fresh event loop for Python 3.12 compatibility
