...
class EvaluationFramework:
    ...
    def evaluate_candidate(self, candidate: Any) -> EvaluationResult:
        """
        Evaluate a candidate solution.
        
        Args:
            candidate: Candidate-like object to evaluate
    ...
    def _parse_candidate_code(self, candidate: Any) -> Dict[str, Any]:
        """Parse and validate candidate code."""
        errors = []
        warnings = []
    ...
    def _evaluate_correctness(self, candidate: Any, parsed_code: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _evaluate_performance(self, candidate: Any, parsed_code: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _evaluate_robustness(self, candidate: Any, parsed_code: Dict[str, Any]) -> Dict[str, Any]:
...
"""
        ...
        # Create a modified candidate with the test function (duck-typed)
        test_candidate = SimpleNamespace(
            id=getattr(candidate, 'id', 'mdhtml_eval'),
            code=full_code,
            prompt=getattr(candidate, 'prompt', ''),
            tools=getattr(candidate, 'tools', {}),
            memory=getattr(candidate, 'memory', {}),
            generation=getattr(candidate, 'generation', 0),
            parent_id=getattr(candidate, 'parent_id', None),
            mutation_type=getattr(candidate, 'mutation_type', 'eval')
        )
...
def create_evaluator(task_type: str) -> Callable[[Any], float]:
    """
    ...
    if task_type == "markdown_to_html":
        ...
        # Default evaluator
        def default_evaluator(candidate: Any) -> float:
...
def create_spec_evaluator(task_spec: Union[TaskSpec, Dict[str, Any]]) -> Callable[[Any], float]:
    """
    Create an evaluator function backed by a concrete TaskSpec.
    Safely wraps EvaluationFramework and returns overall score as fitness.
    
    Args:
        task_spec: Pydantic TaskSpec or a compatible dictionary
    
    Returns:
        Function that maps a Candidate-like object -> float fitness
    """
    framework = EvaluationFramework(task_spec)

    def _evaluate(candidate: Any) -> float:
        try:
            result = framework.evaluate_candidate(candidate)
            return float(result.overall_score)
        except Exception:
            # On any unexpected error, return zero fitness to avoid crashing evolution
            return 0.0
